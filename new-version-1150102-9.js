...
function get_emoji(emojip, sname) {
    var Lmoji = {
        ...
        "💩": ["RU ", "RU-", "RU_", "RUS", "Russia", "俄罗斯", "俄西斯", "Terrorusski", "Trrrorussia", "Ruscism"],
    ...
    var cnt = JSON.parse(str1);  // 始終使用原始 Lmoji，不做任何替換（無論 emojip 是 1 或 2）
    var flag = 0;
    for (var key in cnt) {
    var dd = cnt[key];  // 修正 dd 定義
    for (var i in dd) {
        if (sname.indexOf(dd[i]) != -1) {
            flag = 1;
            var nname = key + " " + sname.replace(/[\uD83C][\uDDE6-\uDDFF][\uD83C][\uDDE6-\uDDFF]/g, "").trim(); // 移除原有旗幟
            return [nname, key];
        }
    }
}
...
// 俄西斯 GIA = custom, ip, 152, aes-128-gcm, password123, https://xxx/download/SSEncrypt.module, obfs=tls, obfs-host=xxx.windows.com, udp-relay=true
